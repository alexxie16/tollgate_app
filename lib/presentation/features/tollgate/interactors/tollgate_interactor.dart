...
class TollgateInteractor {
  ...
  Future<Result<TollgateTopUpResult, String>> topUp({
    ...
    final requestedAmount = BigInt.from(amountSats);
    final storedSwappedTokens =
        await ref.read(swappedEcashOneSatTokensProvider.future);
    final spendableOneSatTokens = storedSwappedTokens
        .where((token) => token.amount == BigInt.one)
        .toList(growable: false);
    final storedOneSatTotal = spendableOneSatTokens.fold<BigInt>(
      BigInt.zero,
      (total, token) => total + token.amount,
    );
    final legacySwappedToken =
        await ref.read(legacySwappedEcashLocalTokenStreamProvider.future);
    final hasExactLegacyToken = legacySwappedToken != null &&
        legacySwappedToken.amount == requestedAmount;

    if (storedOneSatTotal <= BigInt.zero && !hasExactLegacyToken) {
      return Result.failure(
        'No stored local swapped tokens are available for TollGate top-up.',
      );
    }

    if (storedOneSatTotal < requestedAmount && !hasExactLegacyToken) {
      return Result.failure(
        'You need $amountSats sats in stored local swapped tokens, but only $storedOneSatTotal sats are available offline.',
      );
    }
    ...
    try {
      if (hasExactLegacyToken) {
        await ref.read(clearLegacySwappedEcashProvider.future);
        try {
          final paymentResult =
              await ref.read(tollgateServiceProvider).submitEcashToken(
                    cashuToken: legacySwappedToken.encoded,
                    authToken: authToken,
                  );

          switch (paymentResult) {
            case Ok(value: final paymentResponse):
              completedToken = legacySwappedToken;
              lastAuthToken = paymentResponse.authToken ?? authToken;
            case Failure(failure: final failure):
              await ref.read(
                storeLegacySwappedEcashProvider(legacySwappedToken.encoded)
                    .future,
              );
          ...
          await ref.read(
            storeLegacySwappedEcashProvider(legacySwappedToken.encoded).future,
          );
          rethrow;
        }
      } else {
        final selectedTokens = spendableOneSatTokens.take(amountSats).toList();
        var currentStoredTokens = [...storedSwappedTokens];

        for (final token in selectedTokens) {
          final remainingStoredTokens = [...currentStoredTokens]..remove(token);
          await ref.read(
            storeSwappedOneSatTokensProvider(
              remainingStoredTokens.map((token) => token.encoded).toList(),
            ).future,
          );
          ...
          try {
            ...
            switch (paymentResult) {
              case Ok(value: final paymentResponse):
                ...
                lastAuthToken =
                    paymentResponse.authToken ?? lastAuthToken ?? authToken;
                currentStoredTokens = remainingStoredTokens;
              case Failure(failure: final failure):
                await ref.read(
                  storeSwappedOneSatTokensProvider(
                    currentStoredTokens.map((token) => token.encoded).toList(),
                  ).future,
                );
                return Result.failure(failure.message);
            }
      ...
      return Result.failure(
        'Failed to submit the stored local swapped tokens to the TollGate. $error',
      );
    }
  ...
  void _refreshTopUpUi() {
    ref.invalidate(walletHistoryProvider);
    ref.invalidate(currentConnectionStateStreamProvider);
    ref.invalidate(connectivityStreamProvider);
    ref.invalidate(tollgateInfoProvider(kTollgateRouterIp));
    ref.invalidate(tollgateUsageProvider(kTollgateRouterIp));
  }
}
